      ...
      <div className="row">
        ...
        <div className="col">
          <div className="collapse multi-collapse show" id="comics">
            ...
            {(comics.length > 0) ? comics.map((comic, i) => <li key={i} className="list-group-item black-background li-custom">{comic.name}</li>)                
             : 'Has not appeared in comics'}
             {(comics.length > 0 && comicUrl !== '') ? <a href={comicUrl} className="btn custom-button" target="_blank" rel="noreferrer" >Go to Comics</a> : ''}
            </ul>
          </div>
        </div>
        <div className="col">
          <div className="collapse multi-collapse show" id="events">
            ...
            {(events.length > 0) ? events.map((event, i) => <li key={i} className="list-group-item black-background li-custom">{event.name}</li>) : 'Has not appeared in events'}      
            </ul>
          </div>
        </div>
        <div className="col">
          <div className="collapse multi-collapse show" id="series">
            ...
            {(series.length > 0) ? series.map((serie, i) => <li key={i} className="list-group-item black-background li-custom">{serie.name}</li>) : 'Has not appeared in series'}      
            </ul>
          </div>
        </div>
        <div className="col">
          <div className="collapse multi-collapse show" id="stories">
            ...
            {(stories.length > 0) ? stories.map((story, i) => <li key={i} className="list-group-item d-flex justify-content-between align-items-start black-background li-custom"><div>{story.name}</div><span className="badge bg-primary rounded-pill">{story.type}</span></li>) : 'Has not appeared in stories'}      
            </ul>
          </div>
        </div>
      </div>
